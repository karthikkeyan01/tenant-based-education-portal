...
public class OrganizationController {
    ...
    public ApiResponseDto<CreateOrganizationResponseDto> createOrganization(
        ...
        final CreateOrganizationResponseDto response  = this.organizationService.createOrganization(request);
    ...
    public ApiResponseDto<Page<OrganizationResponseDto>> retrieveAllOrganizations(
            @Min(0) @RequestParam(defaultValue = "0") final int page, @Min(1) @Max(100) @RequestParam(defaultValue = "10") final int size){

        final Page<OrganizationResponseDto> response = this.organizationService.retrieveAllOrganizations(page, size);
    ...
    public ApiResponseDto<OrganizationResponseDto> retrieveOrganizationById(@PathVariable final String id) {
        ...
        final OrganizationResponseDto response = this.organizationService.retrieveOrganizationById(id);
    ...
    @PreAuthorize(SecurityConstants.HAS_SUPER_ADMIN)
    @PutMapping("/{id}")
    public ApiResponseDto<OrganizationResponseDto> updateOrganization(@PathVariable final String id,
                                                                      @Valid @RequestBody final OrganizationRequestDto request) {

        final OrganizationResponseDto response = this.organizationService.updateOrganizationById(id, request);
